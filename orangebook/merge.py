...
class OrangeBookMap():
    ...
    def __init__(self):
        self._groups_groupby_NDA = self.__merge_all_csv().groupby(['nda'])
        self._groups_groupby_patent = self.__merge_all_csv().groupby(
            ['patent'])

    def __merge_all_csv(self):
        '''
        Returns data-frame with columns: 'nda' (int64) and 'patent' (object)
        '''
        filelist = sorted(glob.glob('resources/Orange_Book/*/patent.txt'),
                          reverse=True)
        ...
        df_1985_2016 = pd.read_csv(
            'resources/Orange_Book/nber_1985_2016/FDA_drug_patents.csv',
            sep=',').rename(columns=str.lower)

        df_1985_2016_sm = df_1985_2016[['application_number',
                                        'patent_number']].copy()
        df_1985_2016_sm.columns = ['nda', 'patent']

        df_joined = pd.concat([df_obzip_sm, df_1985_2016_sm
    ...
    def get_nda(self, patent):
        ...
        Parameters:
            patent (int or string): the number portion of patent include 'RE'
                                    if applicable
        '''
        return self.__groups_groupby_patent.get_group(
            str(patent))['nda'].tolist()
