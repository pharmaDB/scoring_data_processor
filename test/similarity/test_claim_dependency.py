import unittest
from similarity.claim_dependency import (
    drop_claim_number,
...
class Test_claim_dependency(unittest.TestCase):
    ...
    claims_pre_treat = {
        ...
        3: "3. The gadget of claim 1 further comprising a gizmo (200).\n",
    ...
    claims_no_reference = {
        ...
        3: "3. The gadget of claim 1 further comprising a gizmo.\n",
    ...
    def test_dependent_to_independent_claim(self):
        ...
        """
        independent_claims = dependent_to_independent_claim(
            self.claims_clean, "123456"
        )
        for i in range(1, len(self.claims_clean) + 1):
            self.assertEqual(
    ...
    def test_get_parent_claims(self):
        """
        Test output of get_parent_claims()
        """
        claims_combined = copy.deepcopy(self.claims_clean)
        claims_combined.update(self.sample_claim_preambles_of_alternatives)

        parent_claims = get_parent_claims(claims_combined)

        for i in range(1, len(claims_combined) + 1):
            self.assertEqual(
                parent_claims[i], self.parent_claims_without_text[i]
            )
